...
function TaskCard({ task, state, onMove, onSelect }: { task: WorkspaceTask; state: WorkspaceState; onMove: (status: TaskStatus) => void; onSelect: () => void }) {
  ...
  return (
    <article draggable onDragStart={(event) => event.dataTransfer.setData("text/task", task.id)} className="border border-rule bg-paper-raised p-3 shadow-sm transition-colors hover:border-brass-bright">
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-rule pt-2 font-mono text-[10px] text-ink-3"><span className="truncate">{task.agent}</span><span className="tnum ml-auto shrink-0">{money(task.spentUsd)} / {money(task.budgetUsd)}</span></div>
  ...
  return (
    <aside className="fixed bottom-4 right-4 z-20 max-h-[calc(100vh-2rem)] w-[min(390px,calc(100vw-2rem))] overflow-y-auto border border-rule-strong bg-paper-raised p-4 shadow-xl">
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-rule pt-3">
        <div><dt className="instrument text-[9px]">execution</dt><dd className="mt-1 font-mono text-xs">{attempt?.agentLabel ?? task.agent}</dd></div><div><dt className="instrument text-[9px]">model</dt><dd className="mt-1 truncate font-mono text-xs" title={attempt?.model ?? task.model}>{attempt?.model ?? task.model}</dd></div><div><dt className="instrument text-[9px]">budget</dt><dd className="tnum mt-1 font-mono text-xs">{money(task.spentUsd)} / {money(task.budgetUsd)}</dd></div><div><dt className="instrument text-[9px]">state</dt><dd className="mt-1 font-mono text-xs">{STATUS_LABELS[task.status]}</dd></div>
      </dl>
  ...
  return (
    <div className="min-w-0 flex-1 overflow-y-auto p-6">
      ...
      <section className="mt-6 border border-rule bg-paper-raised">
        ...
        <div className="divide-y divide-rule">{state.tasks.map((task) => { const taskTokens = (task.attempts ?? []).reduce((sum, attempt) => sum + attempt.outputTokens, 0); return <div key={task.id} className="grid grid-cols-[minmax(0,1fr)_90px_90px_120px] items-center gap-3 px-4 py-3"><span className="truncate font-mono text-xs">{task.title}</span><span className="truncate font-mono text-[10px] text-ink-3">{task.agent}</span><span className="tnum text-right font-mono text-[10px] text-ink-3">{taskTokens.toLocaleString()} tok</span><span className="tnum text-right font-mono text-xs">{money(task.spentUsd)} / {money(task.budgetUsd)}</span></div>; })}{!state.tasks.length && <p className="px-4 py-8 text-center font-mono text-xs text-ink-3">Task usage will appear after work is captured.</p>}</div>
      </section>
      <section className="mt-6 border border-rule bg-paper-raised">
...
function Overview({ state, onInspect, onReindex }: { state: WorkspaceState; onInspect: (id: string) => void; onReindex: (path: string) => void }) {
  const files = state.sources.reduce((sum, source) => sum + (source.stats?.files ?? 0), 0);
  ...
  return (
    <div className="min-w-0 flex-1 overflow-y-auto p-6"><div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="border border-rule bg-paper-raised p-5"><div className="flex items-start justify-between gap-3"><div><p className="instrument text-[9px]">active plan</p><h2 className="mt-1 font-mono text-base font-bold">Milestone 3 · coordinated execution</h2></div><span className="font-mono text-[10px] text-moss">{state.sources.length ? 100 : 0}% connected</span></div><p className="mt-3 max-w-[65ch] text-sm leading-relaxed text-ink-2">Connect source context, move work into Ready, and let the coordinator route it through the ACP fleet. Attempts, budgets, model identity, and test evidence remain attached to the task.</p><div className="mt-5 h-2 bg-paper-sunk"><div className="h-full bg-brass-bright" style={{ width: state.sources.length ? "100%" : "8%" }} /></div><div className="mt-4 grid grid-cols-3 gap-3 border-t border-rule pt-3"><Metric label="sources" value={String(state.sources.length)} hint="connected folders" /><Metric label="files" value={String(files)} hint="indexed locally" /><Metric label="edges" value={String(state.edges.length)} hint="traceable links" /></div></section>
      <section className="border border-rule bg-[#0b1014] p-5 text-[#edf0ee]"><div className="flex items-center justify-between"><div><p className="instrument text-[9px] text-[#80909a]">knowledge pulse</p><h2 className="mt-1 font-mono text-base font-bold">Connected sources</h2></div><span className="font-mono text-[10px] text-[#6fbf97]">local index</span></div><div className="mt-5 space-y-2">{state.sources.map((source) => <div key={source.id} className="border border-[#2a353d] p-3"><div className="flex items-center gap-2"><span className="min-w-0 flex-1 truncate font-mono text-xs">{source.label}</span><button type="button" onClick={() => onReindex(source.path)} className="font-mono text-[9px] text-[#d6a05c] hover:underline">reindex</button></div><span className="mt-1 block truncate font-mono text-[10px] text-[#7e8c95]">{source.path}</span><span className="mt-2 block font-mono text-[9px] text-[#98a6ad]">{source.stats?.files ?? 0} files · {source.stats?.relationships ?? 0} relationships{source.stats?.truncated ? " · capped" : ""}</span></div>)}{!state.sources.length && <p className="border border-dashed border-[#34414a] p-4 font-mono text-[10px] leading-relaxed text-[#7e8c95]">No sources connected. Use Connect source above and choose a local repository or notes folder.</p>}</div></section>
    </div><section className="mt-4 border border-rule bg-paper-raised"><header className="flex items-center justify-between border-b border-rule px-4 py-3"><div><p className="instrument text-[9px]">attention queue</p><h2 className="mt-1 font-mono text-sm font-bold">Work that needs a decision</h2></div><span className="font-mono text-[10px] text-ink-3">{attention.length} active</span></header><div className="grid gap-px bg-rule sm:grid-cols-2 lg:grid-cols-4">{attention.map((task) => <button type="button" key={task.id} onClick={() => onInspect(task.id)} className="bg-paper-raised p-4 text-left hover:bg-brass-wash"><span className="instrument text-[9px]">{STATUS_LABELS[task.status]}</span><span className="mt-2 block font-mono text-xs font-semibold">{task.title}</span><span className="mt-2 block font-mono text-[10px] text-ink-3">{task.agent} · {money(task.spentUsd)}</span></button>)}{!attention.length && <p className="col-span-full bg-paper-raised p-6 text-center font-mono text-xs text-ink-3">Capture a task to start the plan.</p>}</div></section></div>
  );
}
...
export function Workspace({ mode, onOpenAgents }: Props) {
  ...
  const receive = (next: WorkspaceState) => { setState(next); saveWorkspace(next); };
  ...
  return (
    <div className="flex min-h-full min-w-0 flex-col bg-paper">
      ...
      {mode === "workspace" && <Overview state={state} onInspect={setSelectedId} onReindex={(path) => void connect(path)} />}
